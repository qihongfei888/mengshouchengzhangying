...
window.Monopoly={
  ...
  getRollCost:function(){
    var input=document.getElementById('monopolyRollCost');
    var cost=input?parseInt(input.value,10):NaN;
    if(!(cost>=0)){
      var cls=_getCurrentClass();
      cost=cls?parseInt(cls.monopolyRollCost,10):NaN;
    }
    if(!(cost>=0)) cost=1;
    return cost;
  },
  loadG:function(){
    var raw=[];try{
      var cls=_getCurrentClass();
      raw=(cls&&Array.isArray(cls.groups))?cls.groups:[];
    }catch(e){}
    var cols=['#FF6B35','#1B98F5','#06D6A0','#9B59F7','#FFD23F','#EF476F'];
    this.groups=raw.map(function(g,i){
      return{id:g.id,name:g.name||'小组',beastId:g.beastId||(window.BEASTS&&window.BEASTS[0]?window.BEASTS[0].id:'qinglong'),icon:g.icon||'🐉',pos:0,score:parseInt(g.points,10)||0,skip:false,color:g.color||cols[i%6]};
    });
  ...
  persistGroupsToClass:function(){
    try{
      ...
      this.groups.forEach(function(g){
        var x=window.app.groups.find(function(k){return k.id===g.id;});
        if(x){x.points=g.score;x.beastId=g.beastId;}
      });
  ...
  setGroupBeast:function(index,beastId){
  ...
  renderSide:function(){
    ...
    if(!this.on){
      this.groups.forEach(function(g,i){
        var opts=(window.BEASTS||[]).map(function(b){return '<option value="'+b.id+'"'+(b.id===g.beastId?' selected':'')+'>'+b.icon+' '+b.name+'</option>';}).join('');
        el.innerHTML+='<div style="margin-top:6px"><label>'+_esc(g.name)+'：</label><select onchange="Monopoly.setGroupBeast('+i+',this.value)">'+opts+'</select></div>';
      });
    }
  ...
  startGame:function(){
    this.loadG();this.build();
    this.groups.forEach(function(g){g.pos=0;g.skip=false;});
    this.turn=0;this.on=true;this.render();
    this.log('🎮 游戏开始！','#FF6B35');
    _speak('神兽大PK游戏开始！');
  },
  resetGame:function(){
    this.on=false;this.turn=0;this.cells=[];
    this.groups.forEach(function(g){g.pos=0;g.skip=false;});
    this.persistGroupsToClass();
    this.render();this.log('🔄 重置','#888');
  },
  rollDice:function(){
    ...
    var rollCost=this.getRollCost();
    if((g.score||0)<rollCost){
      this.log(g.name+' 积分不足，无法掷骰（需'+rollCost+'分）','#EF476F');
      _speak(g.name+'积分不足，无法掷骰');
      this.next();
      return;
    }
    g.score-=rollCost;
  ...
  move:function(idx,steps){
    var g=this.groups[idx],n=this.cells.length,self=this;
    var np=Math.min(g.pos+steps,n-1);
    this.log(g.name+' 掷出'+steps+'，到第'+np+'格',g.color);_speak(g.name+'掷出'+steps+'点');
    var cur=g.pos;
    function step(){cur=Math.min(cur+1,np);g.pos=cur;self.render();if(cur<np)setTimeout(step,200);else self.applyCell(idx);}setTimeout(step,200);
  },
  applyCell:function(idx){
    var g=this.groups[idx],cell=this.cells[g.pos],self=this;
    if(cell.t==='FINISH'){
      this.log(g.name+' 到达终点！','#FFD23F');
      _speak(g.name+'到达终点，获得胜利！');
      this.persistGroupsToClass();
      this.showWin(g);
      return;
    }
